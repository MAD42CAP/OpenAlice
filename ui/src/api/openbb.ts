...
export const marketDataApi = {
  async testProvider(provider: string, key?: string, secret?: string): Promise<{ ok: boolean; error?: string; mode?: 'public' | 'authenticated' }> {
    const res = await fetch('/api/market-data/test-provider', {
      method: 'POST',
      headers,
      body: JSON.stringify({ provider, ...(key ? { key } : {}), ...(secret ? { secret } : {}) }),
    })
    return res.json()
  },
